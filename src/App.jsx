...
export default function App() {
  ...
  useEffect(() => {
    return () => {
      ...
      progressWriteRef.current.forEach(entry => clearTimeout(entry.timer));
      progressWriteRef.current.clear();
    };
  }, []);
  ...
  useEffect(() => {
    // Production ทุกเครื่องมี listeners ตรวจสถานะการเชื่อมต่ออยู่แล้ว
    // ห้ามเขียน config/test ทุกครั้งที่เปิดแอป เพราะเพิ่ม write โดยไม่ช่วยการทำงานจริง
  ...
  function persistScanProgress(boxId, items) {
  ...
  function handleScanProgress(boxId, items) {
    if (!boxId) return;
    const previous = progressWriteRef.current.get(boxId);

    // ปิดลังต้องลบ progress ทันที และยกเลิก trailing write ที่อาจเขียนข้อมูลเก่ากลับมา
    if (items.length === 0) {
      if (previous?.timer) clearTimeout(previous.timer);
      progressWriteRef.current.delete(boxId);
      persistScanProgress(boxId, []);
      return;
    }

    const now = Date.now();
    const entry = previous || { lastWriteAt: 0, pendingItems: null, timer: null };
    const elapsed = now - entry.lastWriteAt;

    // เขียนครั้งแรกทันทีเพื่อให้ Dashboard เห็นลัง แล้ว throttle ครั้งต่อไปไม่เกิน 1 ครั้ง/วินาที
    if (entry.lastWriteAt === 0 || elapsed >= PROGRESS_WRITE_INTERVAL_MS) {
      if (entry.timer) clearTimeout(entry.timer);
      entry.lastWriteAt = now;
      entry.pendingItems = null;
      entry.timer = null;
      progressWriteRef.current.set(boxId, entry);
      persistScanProgress(boxId, items);
      return;
    }

    entry.pendingItems = items;
    if (!entry.timer) {
      entry.timer = setTimeout(() => {
        const current = progressWriteRef.current.get(boxId);
        if (!current) return;
        current.timer = null;
        if (!current.pendingItems) return;
        const latest = current.pendingItems;
        current.pendingItems = null;
        current.lastWriteAt = Date.now();
        persistScanProgress(boxId, latest);
      }, PROGRESS_WRITE_INTERVAL_MS - elapsed);
    }
    progressWriteRef.current.set(boxId, entry);
  }
