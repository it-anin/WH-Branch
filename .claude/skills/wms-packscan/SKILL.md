...
- **Init `items` หักของที่แพ็คไปแล้ว → `buildPackItems({ catalog, boxes, itemsByBox, packer, factorMap })` ใน `units.js`**: `need = catalog.qty × factor − จำนวนที่พนักงานคนนี้แพ็คไปแล้วใน Picklist Run เดียวกัน` (key = `picklistRunId + sku + unit`; รวมเฉพาะลัง status `closed`/`exported`/`received` ที่ `packer.code` ตรงกัน) แล้ว `.filter(need > 0)` — catalog/ลัง legacy ที่ไม่มี Run ID ยังจับคู่กันได้โดยไม่ migration
...
- **กันลังเปิดข้ามรอบ:** ลังใหม่ stamp `picklistRunId`/`picklistRunIds`; เมื่อ catalog เปลี่ยน Run แต่ `activeBoxId` ยังชี้ลังเดิม PackScanC จะปลดลังและบังคับเปิดลังใหม่. `catalogSig` รวม Run ID จึง remount แม้ไฟล์ใหม่มี SKU/หน่วย/จำนวนเหมือนเดิมทุกช่อง
...
- **`handleOpenBox()`** → `await createNewBox()` (เหมือน Desktop's "+ เปิดลังใหม่") + toast "เปิดลังแล้ว ✓" — `createNewBox()` เดิม set `createdAt: Date.now()` อยู่แล้ว = **จุดเริ่ม KPI**
  - **⚠ ห่อ try/catch ไว้ ห้ามเอาออก** — `createNewBox` เป็น Firestore **transaction** บน `config/boxCounter` (ต้องออนไลน์ + auth พร้อม) และ**ตัวมันเองไม่มี error handling**; เดิม `await` เปล่า ๆ ทำให้เวลาล้ม (เน็ตหลุด / anonymous auth ยังไม่พร้อม / `permission-denied`) **เงียบสนิท ไม่มี toast ไม่มีลัง** → พนักงานรายงานว่า "ปุ่มเปิดลังเสีย" ทั้งที่โค้ดปกติ
  - ตอนนี้ catch แล้ว `console.error('handleOpenBox failed:', err?.code …)` + toast แดง `⚠ เปิดลังไม่สำเร็จ · ตรวจอินเทอร์เน็ตแล้วลองใหม่` → กดซ้ำได้เมื่อ auth/เน็ตพร้อม · **จุด auto-open อื่น (`applyScan` fallback / `doClose` ฝั่ง Desktop / ปุ่ม "+ เปิดลังใหม่") ยังไม่มี error path** ถ้าเจออาการเดียวกันฝั่ง Desktop ให้ครอบเพิ่ม
