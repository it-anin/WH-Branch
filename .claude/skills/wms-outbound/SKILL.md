  ...
  - ขวา (detail, grid `1fr 380px`):
    - คอลัมน์ซ้าย: **"รายชื่อสินค้าในลัง"** ตาราง SKU / ชื่อ / **Barcode** / หน่วย / จำนวน / **LOT** / **Exp** / Location (maxHeight 320) — **แตกแถวตาม LOT จริง** ผ่าน helper กลาง `lotRows(l, lotMap)` ที่ใช้ร่วมกับไฟล์ Text (`handleExportBarcode`) → ข้อมูล barcode/LOT/qty ในตารางตรงกับที่จะ export เป๊ะ (SKU เดียวสแกนคนละ LOT = หลายแถว); คอลัมน์ **Exp โชว์เฉพาะเมื่อมีลังที่กรอก exp** (`hasExp`) — แสดงเป็น **ค.ศ. ดิบตามที่พนักงานกรอก** (ไม่แปลง พ.ศ. ต่างจากไฟล์ Text ที่แปลงผ่าน `toBuddhistExp`)
      - **แก้ไขตาราง (`editMode` — ปุ่ม "✎ แก้ไข" / "✓ อนุมัติ" / "✕ ยกเลิก"):** เฉพาะลัง `closed`/`exported` — คลิก "✎ แก้ไข" → `startEdit()` copy boxItems เข้า `editItems` → ตารางเปลี่ยนเป็น input แก้ **จำนวน (number) / LOT / Exp** ต่อแถว + ปุ่ม `×` ลบแถว. **"✓ อนุมัติ" (`handleSaveEdit`)** = filter qty>0, `setItemsByBox`, อัปเดต box `totalQty`/`skuCount`, **set `scannedLots: null` ทุก item** (เพื่อให้ view mode อ่าน qty/lot/exp จาก field ที่แก้ไข ไม่ใช่ `scannedLots` เก่า — เคยเป็นบั๊ก qty ไม่อัปเดต). ไม่แตะ flow อนุมัติเอกสาร (Text→เลขเอกสาร→exported)
        - **⚠ ต้องคำนวณ `gotBase = qty × lookupFactor(factorMap, sku, unit)` ใหม่ทุกครั้ง — ห้ามเอาออก:** ฝั่งสาขาอ่าน `getNeeded = gotBase ?? qty ?? got` (**เอา `gotBase` ก่อนเสมอ**) → ไม่อัปเดต = **แก้จำนวนที่นี่แล้วสาขายังเห็นเลขเดิม** (บั๊กเดิมที่ทำให้ลังซึ่ง factor ผิดซ่อมไม่ได้เลย) · รับ prop `factorMap` มาเพื่อการนี้
        - **⚠ ต้องคูณด้วย factor ของ `scannedUnit` (หน่วยบาร์โค้ดที่ยิงจริง) — ไม่ใช่ `it.unit` (หน่วย Picklist)** เพราะ `qty` = **จำนวนครั้งที่สแกน** (`doClose` เก็บ `qty: it.got`) และหน่วย Picklist มักไม่มีบาร์โค้ดให้ยิงด้วยซ้ำ (`3ลัง`)
          - **เคยพลาดจริง:** picklist `3ลัง` พนักงานยิงบาร์โค้ด `ลัง` 60 ครั้ง → `gotBase=600` **ถูกแล้ว** แต่โค้ดคิด `60 × factor(3ลัง)=30` = **1800** → เขียนทับข้อมูลที่ถูกอยู่ ผิด 3 เท่า (จับได้ก่อนมีคนใช้). helper `countedBase(it)` คุมจุดนี้จุดเดียว
          - ลังเก่าไม่มี `scannedUnit` → **factor 1** ตาม convention เดิมของ PackScanC (`it.scannedUnit ? factorOf(...) : 1`) **ห้าม fallback ไปหน่วย Picklist**
          - **2 จุดที่สแกนในหน้านี้ต้องเซ็ต `scannedUnit` ด้วยเสมอ** ไม่ใช่แค่ `unit`: ช่อง Barcode ต่อแถว + `handleAddByScan` — ไม่งั้น `countedBase` อ่านหน่วยเก่าค้างจากตอนแพ็ค
        - **คำนวณทุกแถว แต่แถวที่ไม่ได้แก้จะได้ค่าเดิมเป๊ะ** (`qty × factor(scannedUnit)` = นิยามของ `gotBase` อยู่แล้ว) ⇒ **กด ✎ แก้ไข → ✓ อนุมัติ เฉย ๆ ไม่เปลี่ยนอะไร — ตั้งใจให้เป็นแบบนั้น**
        - **ระบบเดาแทนคนไม่ได้ว่าในลังมีของเท่าไหร่จริง** — ลังที่ปิดตอน factor ยังผิด **ต้องมีคนเปิดนับ** แล้ว **ยิงบาร์โค้ดหน่วยที่นับในช่อง Barcode ของแถว → พิมพ์จำนวนที่นับได้ → ✓ อนุมัติ** (เช่น ยิง `ลัง` + พิมพ์ 33 → `gotBase = 33 × 10 = 330`)
        - **⚠ ยกเว้นแถวที่ "สแกนปนหน่วย"** (`distinctScanUnits(it) > 1` — `scannedLots` มี `unit` มากกว่า 1 แบบ): `qty` ของแถวแบบนี้คือ**จำนวนครั้งที่สแกนรวมทุกหน่วย** (picklist โหล: สแกน 1 โหล + 12 ชิ้น → `qty=13`, `gotBase=24` จริง) → `qty × factor` = 156 = **พัง** จึง**คงค่าเดิม + toast เตือน** ให้คนตรวจเอง. negative control ยืนยัน: ถอดการกันนี้ออก `gotBase` 24 ถูกเขียนทับเป็น 390
      - **สแกน barcode ในตาราง (edit mode):** (1) **column Barcode ต่อแถว** — สแกน/พิมพ์ + Enter → `lookupUnitByBarcode(barcodeMap, sku, barcode)` เปลี่ยน**หน่วย**ของแถวนั้นอัตโนมัติ (2) **input "🔍 สแกน…เพิ่มสินค้าใหม่"** เหนือตาราง — Enter → `lookupByScan(barcodeMap, catalog, val, nameMap)` (match SKU ตรง หรือ barcode) → SKU มีอยู่แล้ว = qty+1, SKU ใหม่ = เพิ่มแถว (`handleAddByScan`). รับ prop `catalog` + `barcodeMap` + `nameMap`
      - **ชื่อสินค้า fallback chain: `catalog` → `nameMap` (R05.106 ColF) → เลข SKU (ทางสุดท้าย)** — สแกนสินค้า**นอก Picklist วันนั้น** หา `cat` ไม่เจอ เดิมได้เลข SKU เป็นชื่อ **แล้ว `handleSaveEdit` เขียนลง `boxItems` ถาวร** → ติดไปถึงไฟล์ Excel และจอสาขา (ไม่ใช่แค่ปัญหาการแสดงผล) — **catalog ต้องชนะเสมอ ห้ามสลับลำดับ** (Picklist = ของจริงของวันนั้น)
      - **`fixItemName(l, nameMap)` (`units.js`)** heal แถวเก่าที่ `name === sku` ตอน render — ใส่ที่จุด derive 3 แห่ง: `boxItems`, `searchResults`, `handleExportItems` (Excel). ไม่เขียนข้อมูล แต่ `startEdit()` copy `boxItems` ที่ heal แล้ว → **ถ้าพนักงานกด ✎ แก้ไข → ✓ อนุมัติ ชื่อจริงจะถูกบันทึกถาวร** (ผลข้างเคียงที่ตั้งใจ)
      - **⚠ `nameMap` เป็น `{}` บน Android เสมอ** (listener gate ด้วย `isAndroidMode` — ดู skill `wms-import`) → `fixItemName` no-op, `lookupByScan` fallback เป็นเลข SKU เหมือนเดิม. หน้านี้เป็น desktop warehouse-only จึงไม่กระทบ — **แต่ถ้าจะย้าย logic นี้ไปใช้ฝั่ง Android ต้องปลด gate ก่อน ไม่งั้นจะเงียบ ๆ ไม่ทำงาน**
      - **ยังไม่อัป R05.106 หลัง deploy → `nameMap = {}` → พฤติกรรมเดิมเป๊ะ** (ชื่อเป็นเลข SKU) ไม่พัง แค่ยังไม่หายป่วย
