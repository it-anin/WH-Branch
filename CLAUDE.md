...
- writeBatch ลบ: `boxes/*`, `boxItems/*`, `progress/*`, `history/*`, `config/catalog`, `config/barcodeMap`, `config/catalogByPacker`, `config/costMap`, `config/factorMap`, `config/lotMap`, `config/receive`
...
| G (6) | **หน่วยของ qty** (ตรงกับ R05.106 ColG) — ใช้แปลง qty เป็นหน่วยฐานด้วย `factorMap` **ตั้งแต่ import** |

**LOT aggregation logic (2 passes) — แปลงหน่วยฐานตอน import:**
1. แต่ละ row: `qtyBase = ColF × factor(sku, ColG_unit)` (จาก `factorMap` prop — ส่งจาก App.jsx) → รวมต่อ `(SKU, LOT)` → `totals[sku][lot] = sumBase`
2. เก็บเฉพาะ LOT ที่ `sum > 0` → `map[sku] = [{lot, qty: sumBase}, ...]` (**qty เป็นหน่วยฐานแล้ว, ไม่เก็บ unit**)

**⚠ ทำไมไม่เก็บ `unit` ต่อ lot:** R01.119 มี ~27k LOT entries — เก็บ unit (ชื่อหน่วยไทย) ต่อแถวทำให้ `config/lotMap` โต **1.39MB เกินลิมิต Firestore 1 document = 1MB** → `invalid-argument`. แปลงเป็น base ตั้งแต่ import แล้วเก็บแค่ `{lot, qty}` → 0.82MB ผ่าน (`factorMap` ต้องโหลดก่อน คือ import R05.106 ก่อน LOT — ตาม import order อยู่แล้ว; ถ้ายังไม่มี factor → factor=1 ใช้ qty ตามเดิม)
