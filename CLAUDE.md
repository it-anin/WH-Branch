...
- **Layout:** grid `440px 1fr`
  ...
  - ขวา (detail, grid `1fr 380px`):
    - คอลัมน์ซ้าย: **"รายชื่อสินค้าในลัง"** ตาราง SKU / ชื่อ / **Barcode** / หน่วย / จำนวน / **LOT** / **Exp** / Location (maxHeight 320) — **แถวเป็น "หน่วยฐาน"** ผ่าน helper กลาง `baseRows(l, lotMap, factorMap, barcodeMap)` ที่ใช้ร่วมกับไฟล์ Text + Excel (ดู *baseRows* ด้านล่าง) → ข้อมูล barcode/หน่วย/qty/LOT ในตารางตรงกับที่จะ export เป๊ะ; แถวที่แปลงไม่ได้ (unconvertible) โชว์ **⚠ หน้าหน่วย**; คอลัมน์ **Exp โชว์เฉพาะเมื่อมีลังที่กรอก exp** (`hasExp`) — แสดงเป็น **ค.ศ. ดิบตามที่พนักงานกรอก** (ไม่แปลง พ.ศ. ต่างจากไฟล์ Text ที่แปลงผ่าน `toBuddhistExp`)
...
- ปุ่ม "⇩ ส่งออกไฟล์ Text" → export `.txt` แบบ TSV ไม่มี header: `barcode\tจำนวนสินค้า\tทุนสินค้า\t\t\t\t\t\tLOT\tEXP`
  - **🔑 แปลงเป็น "หน่วยฐาน" ก่อนส่งเสมอ — helper `baseRows(l, lotMap, factorMap, barcodeMap)` (module scope, ห่อ `lotRows` เดิม):** ทุกแถวที่ `lookupFactor > 1` (สแกนด้วยหน่วยใหญ่ เช่น ครึ่งโหล=6) → แปลง `qty × factor` + สลับเป็น**บาร์โค้ด/หน่วยของหน่วยฐาน** (`baseUnitOf` จาก `units.js` — หน่วยที่ factor=1 เช่น ขวด, บาร์โค้ดจาก `barcodeMap[sku__baseUnit][0]`) แล้ว **merge แถว (lot+unit) เดียวกัน** (qty รวม, exp เอาตัวแรกที่มีค่า) — เช่น เบิก 22 ขวด สแกน ครึ่งโหล×2 + ขวด×10 → บรรทัดเดียว `[บาร์โค้ดขวด] 22`. ใช้ร่วมทั้ง **ตาราง "รายชื่อสินค้าในลัง" + ไฟล์ Text + Excel รายการลังทั้งหมด** (ตรงกันเป๊ะ 3 จุด)
    - **SKU ที่แปลงไม่ได้** (factor>1 แต่ R05.106 ไม่มีบาร์โค้ดหน่วย factor=1 ของ SKU นั้น) → mark `unconvertible` คงแถวหน่วยเดิม — **ไฟล์ Text: toast error + block ทั้งไฟล์** (`⚠ {sku} ไม่มีบาร์โค้ดหน่วยฐานใน R05.106 · ส่งออกไม่ได้`); ตาราง/Excel แสดงแถวเดิม + ⚠ หน้าหน่วย (read-only บล็อกไม่ได้)
  - **แยกแถวตาม LOT (`item.scannedLots`):** `baseRows` แตกแถวตาม (LOT + หน่วย) ผ่าน `lotRows` ภายใน — SKU สแกนหลาย LOT ยังแยกแถวละ LOT เหมือนเดิม (หลังแปลงหน่วยฐาน lot เดียวกันถูก merge เหลือแถวเดียวต่อ lot)
    - ตัวอย่าง (SKU เดียว สแกน 3 LOT คนละ 1 ชิ้น): `8850304070993\t1\t54.54\t\t\t\t\t\t106760591` / `...\t106760592` / `...\t106760595` (3 แถวแยกกัน)
  - **ลังเก่าก่อนมี `scannedLots`** (`item.scannedLots` ว่าง/ไม่มี) → fallback แถวเดียวต่อ item (`item.lot`/`item.qty`/`item.exp` ตรงๆ) — แล้วผ่านการแปลงหน่วยฐานเหมือนกัน (`scannedUnit` ครึ่งโหล qty 2 → 12 ขวด)
  ...
  - ทุนสินค้า = `costMap[sku__r.unit]` — `r.unit` ผ่าน `baseRows` เป็น**หน่วยฐาน**แล้ว → ได้ทุนต่อหน่วยฐานอัตโนมัติ (เช่นต่อขวด, 0 ถ้ายังไม่ import cost map); active เมื่อ status `closed`/`exported`
...
- **ปุ่ม "⇩ ส่งออกรายการลังทั้งหมด"** (frame-header ขวา, เดิมชื่อ "Export Excel") — export **ทุกลังที่ปิดแล้ว** เป็นไฟล์ `.xlsx` จริงผ่าน SheetJS (`aoa_to_sheet` + `book_new` + `writeFile`) — **ไม่ใช่ HTML-table trick แบบเดิมแล้ว**:
  ...
  - **แถวเป็นหน่วยฐานด้วย `baseRows(l, lotMap, factorMap, barcodeMap)`** (helper เดียวกับตาราง/ไฟล์ Text — ดู *⇩ ส่งออกไฟล์ Text*) — SKU สแกนปนหน่วยถูกแปลง+merge เป็นหน่วยฐานต่อ lot; แถว unconvertible คงหน่วยที่สแกนเดิม (Excel ไม่ block); `Barcode/หน่วย/จำนวน` = `r.barcode`/`r.unit || l.unit`/`r.qty` ต่อแถว
