...
**อนิเมชั่นสลับ tab (Desktop):** `.canvas` มี `key={tab}` → remount ทุกครั้ง + class `.tab-slide` (styles.css) เลื่อนเข้าตามทิศ — `--dir` (+1 แท็บใหม่อยู่ขวา / -1 ซ้าย) คำนวณจากลำดับ `TABS` เก็บใน `tabDirRef` (ref — กัน re-render อื่นเปลี่ยนค่ากลางอนิเมชั่น); `.canvas` ถูกหุ้มด้วย div `overflow-x: clip` **ห้ามเอาออก** — slide แนวนอนบนจอเต็มจะดัน scrollbar วูบ (`clip` ไม่สร้าง scroll container จึงไม่กระทบ sticky/fixed) · `fill: backwards` → จบแล้ว transform = none ไม่ค้าง (กัน popup `position:fixed` เพี้ยน) · เคารพ `prefers-reduced-motion`
...
- PackScanC re-mount ทันทีด้วย `key={packer.code}-${catalogSig(catalogByPacker[packer.code] || catalog)}` (ดู *Remount key ของ PackScanC* ด้านล่าง)

### Remount key ของ PackScanC (`catalogSig` ใน units.js) — ⚠ อย่ากลับไปใช้ `.length`
- **key = `${packer.code}-${catalogSig(packCatalog)}`** (เหมือนกันทั้ง Desktop [App.jsx] และ Android [AndroidApp.jsx]) — `catalogSig` = hash (djb2) จาก `sku+unit+qty+branch` ของทุกรายการที่พนักงานถือ
- **ทำไมไม่ใช่ `.length`:** `items` (state ใน PackScanC) init ครั้งเดียวตอน mount (`useState(() => buildPackItems(...))`) **ไม่มี useEffect resync** เมื่อ prop `catalog` เปลี่ยน → ต้องพึ่ง key remount. `.length` **ชนกันได้**: อัป Picklist เบิกด่วนชุดใหม่ (คนละสาขา) ที่จำนวนรายการ**เท่าเดิม** → key เดิมไม่เปลี่ยน → ไม่ remount → `items` ค้างของเก่า → `processBarcode` หา `items.find(sku===catMatch.sku)` ไม่เจอ (catMatch มาจาก prop ใหม่) → เด้ง **"⚠ ครบแล้ว" สแกนลงลังไม่ได้** (บั๊กจริงที่เจอ 18 ก.ค.)
- **การันตี in-flight data ยังอยู่:** พนักงานที่รายการไม่เปลี่ยน (โซนปกติ ตอนคนอื่นอัปด่วน) → `catalogSig` เท่าเดิม → ไม่ remount → ของที่สแกนค้างในลังไม่หาย (แม่นกว่า `.length` เพราะดูเนื้อหาจริง). ดู memory `packing-inflight-data-must-survive`
- **เสถียร:** `computeCatalogByPacker` filter ตามโซนแบบ deterministic (คงลำดับไฟล์) → import เนื้อหาเดิมซ้ำได้ sig เท่าเดิม ไม่ remount มั่ว
...
## Picklist เบิกด่วน (**ปุ่มอัปโหลดแยกของตัวเอง** — ไม่ detect จากชื่อไฟล์แล้ว)
- **ปุ่ม `📌 อัปโหลดไฟล์ Picklist เบิกด่วน`** อยู่แถวเดียวกับปุ่ม `1 · Picklist` — **อยู่นอกลำดับบังคับ 1→2→3→4 (ไม่มีเลขนำหน้า, ไม่ล็อก กดได้ตลอด)** เพราะเป็นงานแทรกกลางวัน + chip `📌 เบิกด่วน: N รายการ` (นับด้วย `isUrgentItem`)
- **ปุ่ม `1 · Picklist` บล็อกไฟล์ที่ชื่อมี "เบิกด่วน"** (alert บอกให้ใช้ปุ่มเบิกด่วน) — ปุ่มนั้น replace ทั้ง catalog ถ้าไฟล์ด่วนหลุดเข้าไป = **ล้าง Picklist ทั้งวันทิ้ง เหลือแต่รายการด่วน**
- **ชื่อไฟล์: `Picklist_{สาขา}_เบิกด่วน`** — รหัสสาขาต้องอยู่ **ก่อน** คำว่าเบิกด่วนเสมอ (`Picklist_เบิกด่วน_KKL` → branch null → confirm เตือน; ลังด่วนจะ fallback ไปสาขา Picklist ปกติ = สาขาผิด)
- **แทนที่รายการด่วนเดิมทั้งหมด ทุกสาขา (ไม่ใช่ append แล้ว)**: `updated = [...catalog.filter(it => !isUrgentItem(it)), ...mapped]` (+ stamp `item.branch`/`urgent: true`, **เขียน `_meta` เดิมกลับ ไม่แตะ**) → **อัปซ้ำกี่รอบก็ได้ชุดใหม่เสมอ ไม่บวกรายการ/จำนวน**
  - รายการ Picklist **ปกติไม่ถูกแตะ** → เนื้อหาของพนักงานคนอื่นเท่าเดิม → key `${packer.code}-${catalogSig(...)}` ไม่เปลี่ยน → จอไม่ remount **ของที่สแกนค้างรอด แทรกกลางวันได้**; เฉพาะคนที่ tick 📌เบิกด่วน จอรีเซ็ต (confirm เตือนให้ปิดลังค้างก่อน)
    - **⚠ key ต้องเป็น `catalogSig` ไม่ใช่ `.length`** — คนโซนด่วนอัปด่วนสาขาอื่นจำนวนเท่าเดิม `.length` ไม่ขยับ → ไม่ remount → สแกนไม่ได้ (ดู *Remount key ของ PackScanC*)
  - **⚠ ต้องกรองด้วย `isUrgentItem` (units.js) เท่านั้น ห้ามใช้ `zoneOfItem(it) === NOLOC_ZONE`** — zoneOfItem คืน NOLOC_ZONE ให้**รายการปกติที่ location ว่าง**ด้วย → จะลบรายการปกติทิ้งไปด้วย
  - **"แทนที่รายการ" ≠ "รีเซ็ตยอดที่แพ็คแล้ว"** — `buildPackItems` ยังหักของที่แพ็คลงลังปิดแล้ว (match `sku__unit` จาก `itemsByBox` ไม่ผูกกับ catalog) → อัปไฟล์เดิมซ้ำหลังแพ็คไป 4/10 เห็น need = 6 (ถูกแล้ว เหมือน Picklist ปกติอัปซ้ำ)
  - **ด่วน 2 สาขาพร้อมกันไม่รองรับ** — พนักงานที่ tick 📌เบิกด่วน จะถือทั้งคู่ → `resolveBoxBranch` เจอ branch ปน → fallback ไปสาขา Picklist ปกติ = **ลังด่วนได้สาขาผิดเงียบๆ** (การล้างทุกสาขาทำให้เข้าสถานะนี้ยากขึ้น)
- รายการไม่มี location → เห็นเฉพาะคนที่ tick โซน 📌เบิกด่วน · ลังของคนนั้นได้สาขาจากไฟล์ด่วน (`resolveBoxBranch`) — **คนละสาขากับงานปกติได้**
...
- หน้าสแกน (`currentStaff` เลือกแล้ว): header แถวเดียว (ที่ทำงาน + 👤 พนักงาน + ปุ่ม "เปลี่ยน") ต่อด้วย content เต็มพื้นที่ที่เหลือ — `isWarehouse ? <PackScanC> : <BranchReceive branch={branch.code}>`
  - PackScanC ได้ `key={packer.code}-${catalogSig(packCatalog)}` — remount ทุกครั้งที่สลับพนักงานหรือ **เนื้อหา** catalog เปลี่ยน เพื่อ reset state ภายใน (รวม manual LOT form — ดู *LOT Selection*) · **เดิมใช้ `.length` → ชนกันเมื่ออัปด่วนสาขาอื่นจำนวนเท่าเดิม** (ดู *Remount key ของ PackScanC*)
...
## Known Pitfalls
- **Blind receiving รั่วได้หลายทางมาก ไม่ใช่แค่ "ไม่โชว์เลข `needed`"** — เจอและอุดไป 3 รอบใน session เดียว (`BranchReceive`) สิ่งต่อไปนี้**นับเป็น oracle ทั้งหมด**: **สี**แดง/เขียว/เหลืองตอนนับ · **แถวหาย / รายการหดเมื่อนับตรง** · **แบนเนอร์ "ตรวจครบแล้ว"** · **ตัวเลข `needed` บนหน้า result ของแถวที่นับผิด** · **delta `+N` ของที่เกิน** (ถอดสมการหา `needed` ได้) · **ช่องกรอกที่โผล่เฉพาะบางเงื่อนไข** (ใบ้ว่าจำนวน > N) · **dialog ยืนยันที่บอกว่าเหลือกี่รายการ**
  - **เกณฑ์ตัดสิน:** *ถ้าพนักงานเปลี่ยนพฤติกรรมได้จากสิ่งที่เห็น (หยุดยิงพอดี / กรอกเลขให้ตรง) โดยไม่ต้องนับของจริง = รั่ว* — บอก "ผ่าน/ไม่ผ่าน" ได้ แต่ห้ามบอก "เท่าไร"
  - **โหมด pharmacist recheck ต้อง blind เท่ารอบแรกเสมอ** — เคยตั้งใจโชว์สีให้เภสัช (คอมเมนต์เดิม: "เภสัชรู้อยู่แล้วว่าตัวไหนขาด/เกิน") กลายเป็นเภสัชหยุดยิงตอนไฟเปลี่ยน = **ด่านตรวจรอบสองไร้ความหมาย** → ดู skill `wms-receive`
  - **หน้า `result` โชว์ผลได้ แต่ห้ามโชว์ "จำนวนที่ถูกต้อง" ของแถวที่นับผิด** ไม่งั้นรอบรีเช็คได้คำตอบไปฟรี ๆ
- **Firestore write ที่ไม่มีใครดัก error = ปุ่มที่ "กดแล้วไม่มีอะไรเกิดขึ้น"** — `createNewBox` (transaction บน `config/boxCounter`) ไม่มี try/catch และ `handleOpenBox` เดิมก็ `await` เปล่า ๆ → auth ยังไม่พร้อม / เน็ตหลุด / `permission-denied` = **เงียบสนิท** พนักงานสรุปว่า "ปุ่มเปิดลังเสีย" ทั้งที่โค้ดถูก (อุดแล้วด้วย try/catch + toast แดง ใน `PackScanC.handleOpenBox`)
  - **`signInAnonymously(auth).catch(() => {})` ใน `firebase.js` กลืน error ของ auth ทิ้ง** — ถ้า Anonymous provider ถูกปิดใน Firebase console หรือ token ยังไม่มาตอน cold start ทุก write จะ `permission-denied` **แบบไม่มีสัญญาณใด ๆ** แม้อินเทอร์เน็ตปกติ → เวลา debug "กดปุ่มแล้วไม่ทำงาน" ให้เช็ค auth ก่อนเน็ต
  - **transaction ใช้ offline ไม่ได้** (ต้องอ่าน counter ล่าสุด) ต่างจาก write ปกติที่ Firestore queue ไว้ได้
...
- **⚠ `item.qty` ในลัง = "จำนวนครั้งที่สแกน" ไม่ใช่ "จำนวนหน่วย Picklist"** (`doClose` เก็บ `qty: it.got`) — ส่วน `item.unit` = **หน่วย Picklist** ซึ่ง**มักไม่มีบาร์โค้ด** (`3ลัง` ไม่มีใครยิงได้) หน่วยที่ `qty` นับอยู่จริงคือ **`scannedUnit`**
  - **`qty × factor(unit)` จึงผิด — ต้องเป็น `qty × factor(scannedUnit)`** · เคยพลาดจริง: picklist `3ลัง` พนักงานยิงบาร์โค้ด `ลัง` 60 ครั้ง (gotBase=600 ถูกแล้ว) แต่โค้ดคิด `60 × factor(3ลัง)=30` = **1800 เขียนทับข้อมูลที่ถูกอยู่** (จับได้ก่อนมีคนใช้)
  - ลังเก่าไม่มี `scannedUnit` → **factor = 1** ตาม convention เดิม (`PackScanC`: `it.scannedUnit ? factorOf(...) : 1`) **ห้าม fallback ไปใช้หน่วย Picklist**
  - **สแกนปนหน่วย** (`scannedLots` มีหลาย `unit`) → `qty × factor` ใช้ไม่ได้เลยทุกสูตร (qty รวมทุกหน่วย) → คงค่าเดิม + เตือน
  - **ระบบเดาแทนคนไม่ได้ว่าในลังมีของเท่าไหร่** — ถ้ายอดที่สแกนไว้ผิด **ต้องมีคนนับของจริง** แล้วยิงบาร์โค้ดหน่วยที่นับ + พิมพ์จำนวนในหน้า Outbound · **กด ✎ แก้ไข → ✓ อนุมัติ เฉย ๆ ไม่เปลี่ยนอะไร (ตั้งใจ)**
...
- **ลังที่ปิดไปแล้วไม่ได้คำนวณ factor ใหม่อัตโนมัติ และ "ซ่อมอัตโนมัติ" ไม่ได้ด้วย** — `gotBase` ถูกแช่ไว้ใน `boxItems` ตั้งแต่ `doClose` (= จำนวนที่**สแกนไว้จริง**) → แก้ factor แล้วลังเก่ายังค้างเลขเดิม · **แต่ระบบไม่รู้ว่าในลังมีของเท่าไหร่จริง** จะเดาแทนคนไม่ได้ → **ต้องมีคนเปิดลังนับ** แล้วเข้า Outbound → ✎ แก้ไข → **ยิงบาร์โค้ดหน่วยที่นับในช่อง Barcode ของแถว + พิมพ์จำนวนที่นับได้** → ✓ อนุมัติ (รอบเบิกถัดไปถูกเองเพราะ factor ใหม่ใช้ตั้งแต่ตอนแพ็ค)
...
- **`config/catalog`/`catalogByPacker` มีเพดาน ~950KB (guard ใน ImportCatalog onImport, App.jsx)** — เคยเจอจริง 17 ก.ค.: อัปไฟล์เบิกด่วนทดสอบ ~3,000 แถว → append แล้ว doc เกิน 1MB → `invalid-argument` **ล้มทั้ง 2 doc** (ตอนนั้น catalogByPacker ไม่มี .catch = ล้มเงียบ — อุดแล้ว) · guard เช็คขนาด**ก่อนแตะ state** แล้ว toast บอกจำนวน/ขนาด · **catalogByPacker เกินง่ายกว่า catalog** เพราะ duplicate รายการต่อพนักงาน (คนไม่มีโซน = ได้ทั้ง catalog) · picklist งานจริง (16–500 แถว ≈ 5–163KB) ห่างลิมิต ~6–190 เท่า — ถ้าวันหน้ามี use case จริงใหญ่กว่านี้ → shard ตาม pattern lotMap
...
- **key remount ของ PackScanC ห้ามใช้ `.length` — ต้องเป็น `catalogSig` (เนื้อหา)** (bug จริง 18 ก.ค.: โซนเบิกด่วนอัปไฟล์ด่วนสาขาอื่นแล้ว**สแกนไม่ได้ เด้ง "ครบแล้ว"**). `items` state ใน PackScanC init ครั้งเดียวตอน mount ไม่ resync ตาม prop `catalog` → พึ่ง key remount อย่างเดียว; `.length` ชนกันเมื่อชุดใหม่จำนวนรายการเท่าเดิม (เบิกด่วนโหมด replace ทำให้เกิดง่าย) → ไม่ remount → `processBarcode` หาใน `items` เก่าไม่เจอ → เข้าเงื่อนไข `!match`. แก้: `catalogSig` (units.js — hash sku+unit+qty+branch) ทั้ง 2 จุด (App.jsx + AndroidApp.jsx) — รายละเอียด *Remount key ของ PackScanC*
