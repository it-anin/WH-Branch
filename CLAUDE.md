...
- **แทนที่รายการด่วนเดิมทั้งหมด ทุกสาขา (ไม่ใช่ append แล้ว)**: `updated = [...catalog.filter(it => !isUrgentItem(it)), ...mapped]` (+ stamp `item.branch`/`urgent: true`, **เขียน `_meta` เดิมกลับ ไม่แตะ**) → **อัปซ้ำกี่รอบก็ได้ชุดใหม่เสมอ ไม่บวกรายการ/จำนวน**
  - รายการ Picklist **ปกติไม่ถูกแตะ** → จำนวนรายการของพนักงานคนอื่นเท่าเดิม → key `${packer.code}-${length}` ไม่เปลี่ยน → จอไม่ remount **ของที่สแกนค้างรอด แทรกกลางวันได้**; เฉพาะคนที่ tick 📌เบิกด่วน จอรีเซ็ต (confirm เตือนให้ปิดลังค้างก่อน)
